...
do
    ...
    function Funcs:AddColorPicker(Idx, Info)
        ...
        function ColorPicker:Show()
            ...
            -- Dynamically reposition to avoid going off-screen
            local screenHeight = ScreenGui.AbsoluteSize.Y;
            local pickerHeight = PickerFrameOuter.Size.Y.Offset;
            local swatchAbsY = DisplayFrame.AbsolutePosition.Y;
            local swatchAbsX = DisplayFrame.AbsolutePosition.X;
            local swatchHeight = DisplayFrame.AbsoluteSize.Y;
            local parentAbsX = Container.Parent.AbsolutePosition.X;
            local parentAbsY = Container.Parent.AbsolutePosition.Y;
            local localX = swatchAbsX - parentAbsX;
            local localY = swatchAbsY - parentAbsY + swatchHeight + 2;
            -- If it would go below the screen, flip above the swatch
            if swatchAbsY + swatchHeight + 2 + pickerHeight > screenHeight then
                localY = swatchAbsY - parentAbsY - pickerHeight - 2;
            end;
            PickerFrameOuter.Position = UDim2.new(0, localX, 0, localY);
...
do
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        function Toggle:SetDependency(ToggleIdx)
            ...
            local dep = Toggles[ToggleIdx]
            if dep then
                UpdateVisibility(dep.Value)
                local old = dep.Changed
                dep.Changed = function(v)
                    UpdateVisibility(v)
    ...
    function Funcs:AddSearchDropdown(Idx, Info)
        ...
        local ListOuter = Library:Create('Frame', {
            BorderColor3 = Color3.new(0, 0, 0);
            Position = UDim2.new(0, 4, 0, 20 + RelativeOffset + 1 + 20);
            Size = UDim2.new(1, -8, 0, SEARCH_BOX_H + MAX_DROPDOWN_ITEMS * 20 + 2);
            ZIndex = 20;
            Visible = false;
            Parent = Container.Parent;
        });
        ...
        function Dropdown:SetValues(FilterText)
            ...
            ListOuter.Size = UDim2.new(1, -8, 0, SEARCH_BOX_H + listH);
            Scrolling.CanvasSize = UDim2.new(0, 0, 0, Count * 20 + 1);
            Dropdown:Display();
        end;
...
function Library:SetWatermark(Text)
    ...
    local X, Y = Library:GetTextBounds(Stripped, Enum.Font.Code, 14);
