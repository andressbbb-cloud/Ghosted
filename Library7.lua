...
do
    ...
    function Funcs:AddColorPicker(Idx, Info)
        ...
        PickerFrameOuter.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 then
                local ObjPos = Vector2.new(
                    Mouse.X - PickerFrameOuter.AbsolutePosition.X,
                    Mouse.Y - PickerFrameOuter.AbsolutePosition.Y
                );
                while InputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
        ...
        -- DisplayFrame only opens the picker; it cannot reliably close it because
        -- PickerFrameOuter (ZIndex 15, Active=true) overlaps the swatch when open
        -- and steals the click, so this handler never fires while the picker is visible.
        ...
        -- Global handler owns all closing logic, including clicks on the swatch.
        Library:GiveSignal(InputService.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 then
                ...
                -- Close when clicking the swatch (toggle off) or clicking outside the picker
                if overSwatch or not Library:MouseIsOverOpenedFrame() then
                    ColorPicker:Hide();
                end;
            end;
        end))
